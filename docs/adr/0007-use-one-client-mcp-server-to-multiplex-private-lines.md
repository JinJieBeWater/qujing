...
外部 Agent 配置恰好一个本地 Colleague Line Client MCP server，而非每 Owner 一个 MCP server。Client 私有管理多个 Line；Line ID 在记录存在期间稳定，记录删除后可复用。每条 Line 独占 expected Owner ID、Tailcat server address/port/private key、remote Gateway Client ID 和 bearer。
