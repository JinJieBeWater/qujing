# Colleague Line glossary

**Agent**: External MCP consumer. Configures one local Client MCP server.

**Client**: Local Colleague Line process serving one Agent. It owns many private Lines and exposes only `list_lines()` and `ask({ line, workspace, question })`.
